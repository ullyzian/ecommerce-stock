...
def download_file(request, item_id):
    ...
    response = HttpResponse(
        open(filepath, "rb"), content_type="application/force-download"
    )
    response["Content-Disposition"] = f'attachment;filename="{filename}"'
    os.remove(filepath)
    messages.success(request, _("Image downloaded"))
    return response
...
class PaymentView(LoginRequiredMixin, View):
    ...
    def post(self, *args, **kwargs):
        ...
        s3Resource.meta.client.upload_file(
            zip_path, settings.AWS_STORAGE_BUCKET_NAME, f"zip/{zip_filename}"
        )
        ...
        if payment_method == "paypal":
            ...
            try:
                ...
                messages.error(self.request, _("Rate limit error"))
                return redirect("/")

            except stripe.error.InvalidRequestError:
                messages.error(self.request, _("Invalid parametrs"))
                return redirect("/")

            except stripe.error.AuthenticationError:
                messages.error(self.request, _("Not authenticated"))
                return redirect("/")
                ...
                messages.error(
                    self.request, _("Something wen wrong. You were not charged.")
                )
                return redirect("/")

            except Exception:
                messages.error(self.request, _("Serious error is occured."))
                return redirect("/")
                # send email with bug
...
def download_zip(request):
    ...
    os.remove(zip_path)
    messages.success(request, _("Images downloaded"))
    return response
...
def add_to_cart(request, slug):
    ...
    if order_qs.exists():
        ...
        if order.items.filter(item__slug=item.slug).exists():
            messages.info(request, _("This item is already in cart!"))
        # If order item is not in order
        else:
            # Adding an order item to order
            order.items.add(order_item)
            messages.info(request, _("This item has been added to cart!"))
    # If active orders does not exists
        ...
        order.items.add(order_item)
        messages.info(request, _("Order has been created and item has been added!"))
...
def remove_from_cart(request, slug):
    ...
    if order_qs.exists():
        ...
        # If order item is not in order
            ...
            messages.info(request, _("Item does not in the cart to remove it!"))
    # If no active orders exists
    else:
        messages.info(request, _("You do not have order to remove items!"))
